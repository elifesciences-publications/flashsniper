...
PRO ShowProgress::UpDate, percent
  ...
  xyouts, self.xsize/2-15, self.ysize/2-7, string(percent,format='(i0)')+'%', $
    color=240, charsize=1.5, charthick=1.5, /dev
...
;;--------------------------------------------------------------------------------
;;ADDED BY SUNTAO,20151210
PRO ShowProgress::SETPOSITION,XOFF=XOFF,YOFF=YOFF
    Widget_Control, self.tlb, XOffset = XOFF, $
    YOffset = YOFF
END
...
PRO Example
  ...
  autoTimer = Obj_New("ShowProgress", tlb, Color=20, Steps=20, Delay=5, /AutoUpdate, xsize=400, ysize=30)
